...
function getAllDirectors(movies) {
  const directorswithDuplicate = movies.map(function (movie){
    return(movie.director);
  });
  
// unique directors
  let uniquedirectors = directorswithDuplicate.filter(function(director,index){
    return directorswithDuplicate.indexOf(director) === index;
  })

  console.log('directors: ' + uniquedirectors);
  return uniquedirectors;
}

// Iteration 2: Steven Spielberg. The best? - How many drama movies did STEVEN SPIELBERG direct?

function howManyMovies(movies,genretoFind,directorToFind) {
  
  let genre = '';
  let director = '';
  
  //default genre and director to Drama and Steven Spielberg NOT USED
  genre = arguments[1] === null || arguments[1] === undefined ? 'Drama' : genretoFind;
  director = arguments[2] === null || arguments[2] === undefined ? 'Steven Spielberg' : directorToFind;
  
  const dramaMoviesForStephenSpielberg = movies.filter(function(movie){
    console.log(movie.genre);
    return movie.director == 'Steven Spielberg' && movie.genre.indexOf('Drama')>-1;
    
});

return (dramaMoviesForStephenSpielberg.length);
...
function dramaMoviesScore(movies) {
  ...
  avgDrama = scoresAverage(dramaMovies);
...
for (let j = 0; j<sortedMovies.length; j++){
  if(j<20){
    movieTitles.push(sortedMovies[j].title);
  }
}
return movieTitles;
}
...
function turnHoursToMinutes(movies) {
  let myDurHour, myDurMinute, myDur;
  let moviesDur = [];
  
  for(let i in movies){
    moviesDur[i] = movies[i];
  }
  
    for(let i in moviesDur){
      myDurHour = 0;
      myDurMinute = 0;
      if(moviesDur[i].duration.indexOf('h') > -1){
        myDurHour = Number(moviesDur[i].duration.split('h')[0]);
      }
      if(moviesDur[i].duration.indexOf('min') > -1){
        myDurMinute = Number(moviesDur[i].duration.split(' ')[1].split('min')[0]);
      }
    myDur = myDurHour*60 + myDurMinute;
    moviesDur[i].duration = Number(myDur);  
  };
